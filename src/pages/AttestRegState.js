...
import { getAttestStates } from '../redux/apiCalls';
import { ProgressSpinner } from 'primereact/progressspinner';
import { Button } from 'primereact/button';
const AttestRegState = () => {
...
states.map((it,i)=>{
    ...
    <Link to={`/attestation/lga/${it.State_ID}`}>
    <span className='text-green-500 text-xs font-bold'>view lgas</span>
    </Link>
    
    

    <Link to={`/attestation/list?state=${it.State_ID}`}>
